...
export const updateCourse = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const existing = await prisma.trainingCourse.findUnique({ where: { course_id: id } });
...
export const deleteCourse = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const course = await prisma.trainingCourse.findUnique({ where: { course_id: id } });
...
export const getStudents = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const students = await prisma.trainingStudent.findMany({
      where: { deleted_at: null },
...
export const createStudent = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const newStudent = await prisma.trainingStudent.create({
      data: {
        student_name,
        mobile: mobile ? cleanMobile(mobile) : null,
        email,
        course_enrolled,
        batch_name,
...
export const updateStudent = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const existing = await prisma.trainingStudent.findUnique({ where: { student_id: id } });
    if (!existing) return res.status(404).json({ success: false, message: 'Student not found' });

    const updated = await prisma.trainingStudent.update({
      where: { student_id: id },
      data: {
        student_name: student_name || existing.student_name,
        mobile: mobile !== undefined ? (mobile ? cleanMobile(mobile) : null) : existing.mobile,
...
export const deleteStudent = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const student = await prisma.trainingStudent.findUnique({ where: { student_id: id } });
    if (!student) return res.status(404).json({ success: false, message: 'Student not found' });

    await prisma.trainingStudent.update({
      where: { student_id: id },
      data: { deleted_at: new Date() }
    });
...
export const getBatches = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
...
export const updateBatch = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const existing = await prisma.trainingBatch.findUnique({ where: { batch_id: id } });
...
export const deleteBatch = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const batch = await prisma.trainingBatch.findUnique({ where: { batch_id: id } });
...
export const getStudyLogs = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const logs = await prisma.trainingStudyLog.findMany({
      where: { deleted_at: null },
...
export const createStudyLog = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const { log_date, course_name, hours_studied, topics_covered } = req.body;

    const log = await prisma.trainingStudyLog.create({
      data: {
...
export const updateStudyLog = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const { log_date, course_name, hours_studied, topics_covered } = req.body;

    const existing = await prisma.trainingStudyLog.findUnique({ where: { log_id: id } });
    if (!existing) return res.status(404).json({ success: false, message: 'Study log not found' });

    const updated = await prisma.trainingStudyLog.update({
      where: { log_id: id },
      data: {
...
export const deleteStudyLog = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const log = await prisma.trainingStudyLog.findUnique({ where: { log_id: id } });
    if (!log) return res.status(404).json({ success: false, message: 'Study log not found' });

    await prisma.trainingStudyLog.update({
      where: { log_id: id },
      data: { deleted_at: new Date() }
    });
...
export const getTrainingFeeInstallments = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const data = await prisma.trainingFeeInstallment.findMany({ where: { deleted_at: null }, orderBy: { created_at: 'desc' } });
    return res.status(200).json({ success: true, data });
  } catch (error) { next(error); }
};

export const createTrainingFeeInstallment = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const data = await prisma.trainingFeeInstallment.create({
      data: {
        ...req.body,
...
export const updateTrainingFeeInstallment = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const data = await prisma.trainingFeeInstallment.update({
...
export const deleteTrainingFeeInstallment = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    await prisma.trainingFeeInstallment.update({ where: { id: req.params.id }, data: { deleted_at: new Date() } });
...
export const getTrainingAttendance = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
...
export const markTrainingAttendance = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
